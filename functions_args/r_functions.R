...
summary_by_variable_for_publication <- function(dataframe, sample_metric, variable){
    #' Makes a summary table by count and percent of a variable, relative to the Sample)
    #' @param dataframe, the dataframe to explore
    #' @param sample_metric, the samples comparison to look at
    #' @param variable, the variable to compare across samples
    #' @return 2 tables: one is raw numbers with percentage, one as percentage
    # Ensure the variable names are correctly interpreted as symbols
    sample_metric_sym <- sym(sample_metric)
    variable_sym <- sym(variable)

    # Prepare the data
    prepared_data <- dataframe %>%
        select(!!sample_metric_sym, !!variable_sym) %>%
        na.omit() %>%
        count(!!sample_metric_sym, !!variable_sym) %>%
        group_by(!!sample_metric_sym) %>%
        mutate(Percentage = n / sum(n) * 100)

    # For including NAs
    prepared_data_withNA <- dataframe %>%
        select(!!sample_metric_sym, !!variable_sym) %>%
        count(!!sample_metric_sym, !!variable_sym, .drop = FALSE) %>%
        group_by(!!sample_metric_sym) %>%
        mutate(Percentage = n / sum(n) * 100)

    # Combine counts and percentages into a single string
    prepared_data$Combined <- paste(prepared_data$n, "(", round(prepared_data$Percentage, 1), "%)")
    prepared_data_withNA$Combined <- paste(prepared_data_withNA$n, "(", round(prepared_data_withNA$Percentage, 1), "%)")

    # Convert to a wide format for better readability
    combined_df <- prepared_data %>%
        ungroup() %>%
        select(-n, -Percentage) %>%
        spread(!!variable_sym, Combined)

    combined_df_withNA <- prepared_data_withNA %>%
        ungroup() %>%
        select(-n, -Percentage) %>%
        spread(!!variable_sym, Combined)

    list(with_NA = combined_df_withNA, without_NA = combined_df)
}
